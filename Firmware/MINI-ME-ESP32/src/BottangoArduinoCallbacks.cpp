...
// Global variables to track animation requests
volatile int requestedAnimationIndex = -1;  // -1 means no animation requested
volatile bool requestedAnimationLoop = false;
volatile bool stopAnimationRequested = false;
...
namespace Callbacks
{
    ...
    {
        /*
       // Check if we need to stop the current animation
        if (stopAnimationRequested) {
            BottangoCore::commandStreamProvider->stop();
            stopAnimationRequested = false;
            Serial.println("Animation stopped via callback");
        }
    
        // Check if we need to start a new animation
        if (requestedAnimationIndex >= 0) {
            // Start the requested animation
            Serial.print("Starting animation via callback: ");
            Serial.println(requestedAnimationIndex);
        
            BottangoCore::commandStreamProvider->startCommandStream(requestedAnimationIndex, requestedAnimationLoop);
            requestedAnimationIndex = -1;  // Reset the request
        }    
            */  
    }
